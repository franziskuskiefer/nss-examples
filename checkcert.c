...
static int checkInteger(SECItem* intItem, char* fieldName, int verbose) {
  int len, bitlen;
  if (verbose) {
    printf("Checking %s\n", fieldName);
  }
...
static void checkName(CERTName* n, char* fieldName, int verbose) {
  char* v = 0;
  if (verbose) {
    printf("Checking %s\n", fieldName);
  }
...
static SECStatus OurVerifyData(unsigned char* buf, int len,
                               SECKEYPublicKey* key, SECItem* sig,
                               SECAlgorithmID* sigAlgorithm) {
  SECStatus rv;
  VFYContext* cx;
  ...
  if (sigAlgOid == 0) return SECFailure;
  ...
  if (showDigestOid) {
    ...
    if (oiddata) {
      ...
      SECU_PrintAsHex(stdout, &oiddata->oid, "PROBLEM: UNKNOWN OID", 0);
    }
  }
  ...
  if (rv == SECSuccess) {
    ...
    if (rv == SECSuccess) rv = VFY_End(cx);
  }
...
static CERTCertificate* createEmptyCertificate(void) {
  PLArenaPool* arena = 0;
  CERTCertificate* c = 0;
...
int main(int argc, char** argv) {
  ...
  rv = SEC_ASN1DecodeItem(arena, signedData,
                          SEC_ASN1_GET(CERT_SignedDataTemplate), &derCert);
  if (rv) {
  ...
  rv = SEC_ASN1DecodeItem(arena, cert, SEC_ASN1_GET(CERT_CertificateTemplate),
                          &signedData->data);
  if (rv) {
  ...
  if (rv) {
    fprintf(stderr,
            "%s: failed to set algorithm ID for "
  ...
  if (rv) {
    fprintf(
        stderr,
        "%s: failed to set algorithm ID for SEC_OID_PKCS1_RSA_ENCRYPTION.\n",
        progName);
    exit(1);
  }
  ...
  rsapubkey =
      (SECKEYPublicKey*)PORT_ArenaZAlloc(arena, sizeof(SECKEYPublicKey));
  if (!rsapubkey) {
  ...
  if (rv) {
    ...
    checkInteger(&rsapubkey->u.rsa.publicExponent, "Public Exponent", verbose);
  ...
  if (issuerCert) {
    SECComparison c = CERT_CompareName(&cert->issuer, &issuerCert->subject);
    if (c) {
